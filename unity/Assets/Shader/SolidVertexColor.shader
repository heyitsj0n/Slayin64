...
SubShader {
	...
	Pass {
		...
		fixed4 frag_mult(v2f_vct i) : SV_Target { fixed4 col = tex2D(_MainTex, i.texcoord) * i.color; col.a = 1; return col; }
		ENDCG
	}
}
SubShader {
	Tags {"Queue"="Geometry" "RenderType"="Opaque"}
	ZWrite On Cull Off Fog { Mode Off } LOD 100
	BindChannels { Bind "Vertex", vertex Bind "TexCoord", texcoord Bind "Color", color }
	Pass { Lighting Off SetTexture [_MainTex] { combine texture * primary } }
}
}
